...
composeFunction = (func, emitter, parent) ->
	return () ->
		...
		return func.call parent, emitter, next
...
class Handler
	...
	getHandler: (handler) ->
		func = undefined
		phandler = handler.entity || handler
		...
		return composeFunction func, @.emitter, @.lhandler
	...
	getHandlerByString: (handler) ->
		...
		@.lhandler = lhandler = typesFunctions[typeOfHandler] pathToHandler
